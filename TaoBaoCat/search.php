...
$catid = !($_GET['catid'])?'0':intval($_GET['catid']);
...
$Taoapi->page_no = $page;
$Taoapi->page_size = '32';
$Taoapi->keyword = iconv("GBK","UTF-8",$q);
$Taoapi->sort = $sort;
...
<title><?php echo $q ?>-<?php echo iconv("UTF-8","GBK",$cat_name) ?>-<?php echo $sitetitle ?></title>
<meta name="keywords" content="<?php echo $q ?>,<?php echo iconv("UTF-8","GBK",$cat_name) ?>,<?php echo $sitekey ?>" />
<meta name="description" content="欢迎在<?php echo iconv("UTF-8","GBK",$cat_name) ?>类商品中搜索<?php echo $q ?>产品。<?php echo $sitedesc ?>" />
...
include("header.php");
?>
	<div class="weizhi"><span>您的位置：</span><a href="/">首页</a><a href="list.php">全部分类</a><a href="list.php?catid=<?php echo $catid ?>"><?php echo iconv("UTF-8","GBK",$cat_name) ?></a> <?php echo $q ?></div>
	<div class="taoke_mainbox">
		...
		<div class="taoke_mainboxr">
			...
			<div class="taoke_choose">
				<ul>
					<li<?php if($sort == commissionNum_desc) { echo ' id="this_choose"';} ?>><a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid; ?>&sort=commissionNum_desc">销量从高到低</a></li>
					<li<?php if($sort == credit_desc) { echo ' id="this_choose"';} ?>><a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid; ?>&sort=credit_desc">信用从高到低</a></li>
					<li<?php if($sort == price_desc) { echo ' id="this_choose"';} ?>><a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid; ?>&sort=price_desc">价格从高到低</a></li>
					<li<?php if($sort == price_asc) { echo ' id="this_choose"';} ?>><a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid; ?>&sort=price_asc">价格从低到高</a></li>
					<li<?php if($sort == delistTime_desc) { echo ' id="this_choose"';} ?>><a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid; ?>&sort=delistTime_desc">时间从高到低</a></li>
					<li<?php if($sort == commissionRate_desc) { echo ' id="this_choose"';} ?>><a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid; ?>&sort=commissionRate_desc">比率从高到低</a></li>
				</ul>
			</div>
			<div class="condition">
				<ul>
					...
					<li class='by-price'>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=1&ep=100">1～100元</a>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=100&ep=200">100～200元</a>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=200&ep=500">200～500元</a>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=500&ep=1000">500～1000元</a>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=1000&ep=2000">1000～2000元</a>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=2000&ep=5000">2000～5000元</a>
					<a href="search.php?q=<?php echo urlencode($q) ?>&catid=<?php echo $catid ?>&sort=<?php echo $sort ?>&sp=5000&ep=10000">5000元以上</a>
					</li>
				</ul>
			</div>
			...
			<div id="pages">
				...
				$sub_pages=10;
				$pageCurrent=$page;
				$subPages=new SubPages($page_size,$nums,$pageCurrent,$sub_pages,"search.php?q=".urlencode($q)."&catid=".$catid."&sort=".$sort."&sp=".$sp."&ep=".$ep."&page=",2);
				?>
			</div>
		</div>
		<div class="clear"></div>
